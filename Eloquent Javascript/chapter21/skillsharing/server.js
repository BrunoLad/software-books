...
class SkillShareServer {
    ...
    persistData() {
        writeFile(this.#db, JSON.stringify(this.talks))
            .then((res) => console.log('Data persisted successfully.'))
            .catch((err) => console.log(`Error persisting data: ${err}`));
    }
    #loadData() {
        readFile(this.#db, 'utf-8')
            .then((data) => {
                const parsedData = JSON.parse(data);
                // Could've also been done this way
                // return Object.assign(Object.create(null), json);
                for (let key in parsedData) {
                    this.talks[key] = parsedData[key];
                }
            })
            .catch((err) =>
...
const server = new SkillShareServer(Object.create(null)).start(8000);
